...
def test_calc_root_soil_resistance_decreases_as_root_depth_increases():
    res = [soil.calc_root_soil_resistance(soil_conductivity=1, root_radius=0.0001, root_length_density=2000)
...
def test_calc_root_soil_resistance_decreases_as_soil_conductivity_increases():
    res = [soil.calc_root_soil_resistance(soil_conductivity=v, root_radius=0.0001, root_length_density=2000)
           for v in arange(0, 1, 0.01)]
    assert all(x >= y for x, y in zip(res, res[1:]))


def test_calc_root_soil_resistance_decreases_as_root_radius_increases():
    res = [soil.calc_root_soil_resistance(soil_conductivity=1, root_radius=v, root_length_density=2000)
           for v in arange(0, 0.001, 0.0001)]
    assert all(x >= y for x, y in zip(res, res[1:]))


def test_calc_root_soil_resistance_decreases_as_root_length_increases():
    res = [soil.calc_root_soil_resistance(soil_conductivity=1, root_radius=0.0001, root_length_density=v)
...
def test_calc_collar_water_potential_decreases_as_transpiration_flux_increases():
    ...
    root_params.update(
        {'root_length': 0.99 * root_params['rhyzosphere_volume'] / (3.14 * root_params['root_radius'] ** 2)})
