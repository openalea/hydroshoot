...
from alinea.topvine import V3Dutils, primitive
...
def vector_rotation(vector,axis, theta):
    ...
    """
#    TODO: This function must be added to the 'V3Dutils' package: alinea.topvine.V3Dutils
...
#    incli_init = incli_I + a_insert*scipy.cos(rand_rotate)
    dx, dy, dz = V3Dutils.PolToXyz((len_I, azi_I, incli_I))
    ...
    len_init, azi_init, incli_init = V3Dutils.XyzToPol(vec_2)
...
#                               Getting vector of the previous axisII insertion internode
                                ...
                                len_sg,azi_sg,incli_sg = V3Dutils.XyzToPol((dx_sg,dy_sg,dz_sg))
...
#                               Getting the vector of the primary internode holding the previous axisII
                                ...
                                len_gp,azi_gp,incli_gp = V3Dutils.XyzToPol((dx_gp,dy_gp,dz_gp))
                                ...
                                len_f,azi_f,incli_f = V3Dutils.XyzToPol((dx_f,dy_f,dz_f))
...
#                               First correction azi and incli follow the dirction of the holding primary internode.
                                ...
                                vector = V3Dutils.PolToXyz((length,azi_tempo1,incli_tempo1))
...
#                               Second correction: the axisII insertion internode must adhere to phyllotaxis rule.
                                ...
                                #len_init,azi_init,incli_init = V3Dutils.XyzToPol((dx,dy,dz))
...
#                               Third correction: Considering the uncertainty in the insertion angle
                        ...
                        if counter ==0:
                            ...
                            dx, dy, dz = V3Dutils.PolToXyz((length,azi_init, incli_init))
                        ...
                        len_prec,azi_prec,incli_prec = V3Dutils.XyzToPol(vector_prec)
...
def VinePetiole(g, vid, pet_ins=90., pet_ins_cv=10., phyllo_angle=180.,
    ...
    if vid > 0:
        ...
        if n.label.startswith('inI'):
            ...
            len_f,azi_f,incli_f = V3Dutils.XyzToPol(vec_f)

            vec_gp = scipy.subtract(grandpa.TopPosition,grandgrandpa.TopPosition)
            len_gp,azi_gp,incli_gp = V3Dutils.XyzToPol(vec_gp)
            ...
            if in_order == 1:
                ...
                for sibling in grandpa.children():
                    if sibling.label.startswith('PetI'):
                        ...
                        len_sg,azi_sg,incli_sg = V3Dutils.XyzToPol(vec_sg)
    ...
    try:
        ...
        if n.label.startswith(('inT','cx','inT3y','inT2y','inI','Pet')):
            ...
            if hasattr(n, 'TopPositionPoints'): # If leaf tips are digitized
                ...
                leaf_len, leaf_azi, leaf_incli = V3Dutils.XyzToPol(leaf_vec)
#                theta_1 = -scipy.pi/2.
...
#       Leaves
            ...
            if not hasattr(n, 'TopPositionPoints'):
                ...
                len_lim, azi_lim, incli_lim = V3Dutils.XyzToPol(vec_lim)
